 ...
 * 计算部门任务统计
 */
export async function calculateDepartmentTaskStats(departmentId) {
    // 获取部门下所有项目ID
 ...
 * 计算部门项目统计
 */
export async function calculateDepartmentProjectStats(departmentId) {
    const projects = await prisma.project.findMany({
        where: { departmentId, deletedAt: null },
        select: { id: true }
    });
 ...
 * 计算成员工作负载
 */
export async function calculateMemberWorkload(userId) {
    const tasks = await prisma.task.findMany({
        where: {
            assigneeId: userId,
            deletedAt: null,
