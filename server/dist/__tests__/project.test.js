    ...
    describe('GET /api/projects - 获取项目列表', () => {
        describe('获取用户项目列表', () => {
            ...
            it('项目列表应该包含项目基本信息', async () => {
                const response = await request(app)
                    .get('/api/projects')
                    .set('Authorization', `Bearer ${ownerToken}`);
                const project = response.body.data.find((p) => p.id === projectId);
            ...
            it('非成员用户不应该看到私有项目', async () => {
                ...
                const privateProject = response.body.data.find((p) => p.id === privateProjectId);
        ...
        describe('获取公开项目列表', () => {
            it('应该返回所有公开项目', async () => {
                ...
                expect(response.body.success).toBe(true);
                expect(Array.isArray(response.body.data)).toBe(true);
                const publicProjects = response.body.data.filter((p) => p.visibility === 'PUBLIC');
        ...
        describe('项目负责人删除项目', () => {
            ...
            it('删除后的项目不应该出现在列表中', async () => {
                const response = await request(app)
                    .get('/api/projects')
                    .set('Authorization', `Bearer ${ownerToken}`);
                const deletedProject = response.body.data.find((p) => p.id === deleteProjectId);
        ...
        describe('项目邀请', () => {
            ...
            it('用户应该能够接受邀请', async () => {
                ...
                const membersResponse = await request(app)
                    .get(`/api/projects/${projectId}/members`)
                    .set('Authorization', `Bearer ${ownerToken}`);
                const isMember = membersResponse.body.data.some((m) => m.userId === otherId);
