 ...
 * 获取公开项目列表
 */
export async function getPublicProjects(_req, res) {
    const projects = await prisma.project.findMany({
        where: {
            visibility: 'PUBLIC',
            deletedAt: null
 ...
 * 获取已删除的项目列表
 */
export async function getDeletedProjects(req, res) {
    const userId = req.userId;
    // 计算30天前的日期
 ...
 * 永久删除项目
 */
export async function permanentDeleteProject(req, res) {
    const { id } = req.params;
    const userId = req.userId;
    const project = await prisma.project.findFirst({
        where: { id }
 ...
 * 移交项目负责人
 */
export async function transferProject(req, res) {
    const { id } = req.params;
