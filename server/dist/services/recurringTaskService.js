 ...
 * @returns 下一个重复日期
 */
export function calculateNextDueDate(currentDate, repeatType) {
 ...
 * @returns 新创建的任务或null
 */
export async function createNextRecurringTask(taskId) {
    try {
        // 获取原任务信息
        ...
        const repeatType = originalTask.repeat;
 ...
 */
export async function processCompletedRecurringTasks() {
    try {
        // 查找所有已完成的重复任务
        ...
        for (const task of completedRecurringTasks) {
            ...
            if (!originalTask)
                continue;
            const nextDueDate = calculateNextDueDate(originalTask.dueDate, originalTask.repeat);
            // 检查是否已存在下一个实例
 ...
 * @returns 重复实例列表
 */
export async function getRecurringTaskInstances(taskId) {
