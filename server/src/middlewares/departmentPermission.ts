...
export async function checkProjectAccess(userId: string, projectId: string): Promise<boolean> {
  ...
  // 同部门项目
  if (user.departmentId && project.departmentId === user.departmentId) return true
 ...
 * - 部门管理员可以管理本部门项目
 */
export async function canManageDepartmentProject(userId: string, projectId: string): Promise<boolean> {
  const user = await prisma.user.findUnique({
    where: { id: userId },
    select: { isAdmin: true }
  })
 ...
 * 部门管理员权限中间件（针对特定部门）
 */
export async function requireDeptAdminForProject(req: AuthRequest, res: Response, next: NextFunction) {
